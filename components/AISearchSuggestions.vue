  ...
  <div class="relative">
    ...
    <div class="relative">
      ...
      <div class="absolute left-4 top-1/2 transform -translate-y-1/2">
        <Icon 
          :name="isProcessing ? 'heroicons:sparkles-20-solid' : 'heroicons:magnifying-glass-20-solid'" 
    ...
    >
      ...
      <div v-if="categories.length > 0 && !searchQuery" class="p-3">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Categorias Populares</p>
      ...
      <div v-if="suggestions.length > 0" class="p-3">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Sugestões Inteligentes</p>
        <div class="space-y-1">
          <button
          ...
          >
            <div class="flex items-center gap-3">
              <Icon :name="suggestion.icon" class="w-4 h-4 text-gray-400 group-hover:text-blue-500" />
              <div>
...
const suggestions = computed(() => {
  ...
  if (query.includes('bmw') || query.includes('audi') || query.includes('mercedes')) {
    mockSuggestions.push({
      ...
      filters: { make: query.includes('bmw') ? 'bmw' : query.includes('audi') ? 'audi' : 'mercedes' }
    })
  }
...
const processAIQuery = async () => {
  ...
  if (query.includes('esportivo') || query.includes('rápido') || query.includes('performance')) {
    ...
    filters.push(
      { key: 'yearMin', label: 'Ano mínimo: 2023', value: '2023' }
    )
  } else {
