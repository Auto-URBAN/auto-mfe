<template>
	<div class="space-y-4">
		...
		<div
			...
			"
			class="space-y-3 overflow-y-scroll max-h-56"
		>
		...
		<div v-else-if="currentState === 'models' && selectedBrand" class="space-y-3">
			<div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
				<div class="flex items-center gap-2">
					...
					<span class="text-xs text-blue-600"
						>{{ availableModels.length }} modelos disponíveis</span
					>
					<span
						v-if="selectedModelsForCurrentBrand.length > 0"
						class="text-xs text-green-600 ml-auto"
					>
						{{ selectedModelsForCurrentBrand.length }} selecionado{{
							selectedModelsForCurrentBrand.length > 1 ? 's' : ''
						}}
					</span>
				</div>
			</div>
			...
			<div class="flex gap-2 pt-3 border-t">
				...
				>
					...
					Adicionar {{ selectedModelsForCurrentBrand.length }} modelo{{
						selectedModelsForCurrentBrand.length > 1 ? 's' : ''
					}}
				</UiButton>
			</div>
