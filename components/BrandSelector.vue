<template>
	<div class="space-y-3">
		...
		<div v-if="selectedBrands.length > 0" class="flex flex-wrap gap-2">
			...
			>
				<img :src="brand.logo" :alt="brand.name" class="w-4 h-4 object-contain" loading="lazy" />
			...
			>
				...
				<div class="flex flex-col items-center space-y-2">
					...
					<div class="text-center">
						<div
							class="text-sm font-medium text-gray-900 group-hover:text-green-700 transition-colors"
						>
							{{ brand.name }}
						</div>
...
const toggleBrand = (brandId: string) => {
	...
	emit(
		'change',
		props.brands.filter(b => newSelection.includes(b.id))
	)
}
