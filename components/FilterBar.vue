		...
		<USlideover v-model="showMobileFilters" side="bottom">
			<div class="p-4">
				<div class="flex justify-between items-center mb-4">
					...
					<UButton @click="showMobileFilters = false" color="gray" variant="ghost" size="sm" square>
				...
				<div class="space-y-4">
					...
					<div class="grid grid-cols-2 gap-4">
						<div>
							...
							<UInput v-model="filters.priceMin" placeholder="R$ 0" type="number" />
						</div>

						<div>
							<label class="block text-sm font-medium mb-2">Preço máximo</label>
							<UInput v-model="filters.priceMax" placeholder="R$ 999.999" type="number" />
						</div>
					</div>
					...
					<div class="grid grid-cols-2 gap-4">
						<div>
							...
							<UInput v-model="filters.yearMin" placeholder="2000" type="number" />
						</div>

						<div>
							<label class="block text-sm font-medium mb-2">Ano máximo</label>
							<UInput v-model="filters.yearMax" placeholder="2024" type="number" />
						</div>
					</div>
				</div>

				<div class="flex gap-3 mt-6">
					<UButton @click="clearAllFilters" color="gray" variant="outline" class="flex-1">
						Limpar
					</UButton>

					<UButton @click="applyFilters" color="primary" class="flex-1"> Aplicar </UButton>
				</div>
			</div>
		</USlideover>
	</div>
