<template>
	<div class="relative min-w-[350px] lg:min-w-[700px]">
		<div
			class="bg-white w-full py-2 px-4 rounded-full shadow-lg border flex justify-center items-center outline-4 outline-offset-2 relative z-10"
		>
		...
		<div
			class="bg-white border rounded-xl shadow-xl absolute top-[30px] w-full pt-12"
			v-if="searchListCars.length"
		>
			<router-link
				:to="`/valor-do-carro/${car.slug}`"
				class="flex items-center p-4 text-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
				v-for="(car, idx) in searchListCars"
				:key="car.name"
			>
...
watch(search, value => {
	if (value.length > 1) {
		searchListCars.value = listMock
	} else {
		searchListCars.value = []
	}
})
</script>
